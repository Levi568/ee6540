...
void cleanup();  //required function for cleaning up the framework
#endif
...
int main(){
    ...
    cl_platform_id* platforms;  //platform identifier
    ...
    FILE *fp;  //used for kernal source code
...
#error "unknown OpenCL SDK environment"
    ...
    kernel = clCreateKernel(program, "matrix_calc", &ret);
    if (ret != CL_SUCCESS) {
